...
pipeline {
  ...
  environment {
     APP_NAME = 'BuyRequest/dashboard-api'
     WORKSPACE = '/home/Jenkins/workspace/BuyRequest/dashboard-api'
  }
   agent none
   stages {
            ...
            steps {
                script {
                    sshagent (credentials: ['buyrequestdev']) {
                        ...
                        && cat .env \
                        && cd ../dashboard-api && git checkout -f && git pull \
                        && cd /home/ubuntu/buy-request/docker && make api-restart && make nginx-restart \
                        && make api-logs-without-tail \
